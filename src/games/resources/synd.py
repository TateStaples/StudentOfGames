...
class CounterStrategy:
    ...
    def findCallProb(self, d1, hist):
        ...
        if len(hist) % 2 == 1:
            ...
            xhis = self.zvs[d1, hist].solution_value()
            xpar = self.zvs[d1, hist[:-2]].solution_value()
    ...
    for d1 in ROLLS1:
        for hist in histories():
            ...
            if not hist:
                ...
                print(f'Roll: {d1}, Expected: {avgValue}, Values: {values}')
                continue
            # if len(hist)>2:
            #     continue
            ...
            if len(hist)%2==1:
                ...
                tag = ''#'.join(round(cs.findCallProb(d2,hist),2) for d2 in ROLLS2)
                print(s, tag)
def main():
